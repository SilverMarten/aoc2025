...
public class Day02 {
    ...
    public static void main(String[] args) {
        ...
        var expectedTestResult = 1_227_775_554;
    ...
    private static List<Long> findDoubledIDs(Range<Long> range) {
        ...
        try {
            halfRange = Range.of(Integer.valueOf(min.substring(0,(int) Math.round(min.length() / 2.))),
                                 Integer.valueOf(max.substring(0,(int) Math.round(max.length() / 2.))));
        } catch (NumberFormatException nfe) {
            log.error("Could not create a range from {} ({}-{})", range, min, max, nfe);
            return Collections.emptyList();
        }
        log.trace("Checking {}", halfRange);

        return IntStream.rangeClosed(halfRange.getMinimum(), halfRange.getMaximum())
                        .mapToLong(i -> Long.valueOf("%d%1$d".formatted(i)))
